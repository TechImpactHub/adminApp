...
const AddVendorProfile = () => {
    ...
    // 
    const [addVendor] = useMutation(GET_PARTNER_MUTATION, {
      variables: {
      ...
      onCompleted: (partner) => {
        ...
        if (loaded_partner !== null) {
            let p_vendor = loaded_partner.vendorSet.edges;
            console.log(loaded_partner)
            if (p_vendor.length === 0) {
                history.push('/admin/add-vendor-school', partner)
            }
            else {
                p_vendor.map(vendor => 
                    setFormState({
                    ...
                    <CardBody>
                      ...
                      <Form>
                        ...
                        <Row>
                            ...
                            {formState.partnerResults ? <p>Partner of ID {formState.nationalId} not found</p> : (
                                <></>
                            )}
                            {formState.vendorExists ? <p>Vendor of ID {formState.nationalId} exists</p> : (
                                <></>
                            )}
